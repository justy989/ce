...
bool ce_vim_init(CeVim_t* vim){
     ...
     ce_vim_add_key_bind(vim, '\'', &ce_vim_parse_motion_mark);
     ...
     ce_vim_add_key_bind(vim, 'm', &ce_vim_parse_verb_set_mark);
               ...
               if(ce_buffer_insert_rune(view->buffer, key, view->cursor)){
                    ...
                    if(key == CE_NEWLINE){
                         ...
                         if(string_is_whitespace(view->buffer->lines[save_cursor.y])){
                              ...
                              char* remove_string = ce_buffer_dupe_string(view->buffer, remove_loc, remove_len);
                              if(ce_buffer_remove_string(view->buffer, remove_loc, remove_len)){
                                   change.chain = true;
     ...
     case KEY_BACKSPACE:
          if(!ce_points_equal(view->cursor, (CePoint_t){0, 0})){
               ...
               if(view->cursor.x == 0){
                    ...
                    removed_string = ce_buffer_dupe_string(view->buffer, remove_point, remove_len);

                    if(ce_buffer_remove_string(view->buffer, remove_point, remove_len)){
                         // TODO: convenience function
                         CeBufferChange_t change = {};
                         change.chain = vim->chain_undo;
                         change.insertion = false;
     ...
     case CE_VIM_MODE_REPLACE:
          if(key != CE_NEWLINE && key != 27){ // escape
               ...
               if(view->cursor.x < last_index){
                    int64_t remove_len = 1;
                    char* removed_string = ce_buffer_dupe_string(view->buffer, view->cursor, remove_len);

                    if(ce_buffer_remove_string(view->buffer, view->cursor, remove_len)){
                         // TODO: convenience function
                         CeBufferChange_t change = {};
                         change.chain = vim->chain_undo;
                         change.insertion = false;
...
bool ce_vim_join_next_line(CeBuffer_t* buffer, int64_t line, CePoint_t cursor, bool chain_undo){
     ...
     if(!ce_buffer_remove_string(buffer, point, 1)) return false;
...
static bool vim_motion_find(CeVim_t* vim, CeVimAction_t* action, const CeView_t* view, const CeConfigOptions_t* config_options,
                            CeVimMotionRange_t* motion_range, bool inside){
...
bool ce_vim_motion_mark(CeVim_t* vim, CeVimAction_t* action, const CeView_t* view, const CeConfigOptions_t* config_options,
                        CeVimBufferData_t* buffer_data, CeVimMotionRange_t* motion_range){
     CePoint_t* destination = buffer_data->marks + ce_vim_yank_register_index(action->motion.integer);
...
bool ce_vim_verb_unindent(CeVim_t* vim, const CeVimAction_t* action, CeVimMotionRange_t motion_range, CeView_t* view,
     ...
     for(int64_t i = motion_range.start.y; i <= motion_range.end.y; i++){
          ...
          if(strncmp(view->buffer->lines[i], remove_string, config_options->tab_width) == 0){
               // insert indentation
               if(!ce_buffer_remove_string(view->buffer, indentation_point, config_options->tab_width)) return false;
...
bool ce_vim_verb_join(CeVim_t* vim, const CeVimAction_t* action, CeVimMotionRange_t motion_range, CeView_t* view,
     ...
     if(whitespace_len > 0){
          char* whitespace_str = ce_buffer_dupe_string(view->buffer, beginning_of_next_line, whitespace_len);
          if(!ce_buffer_remove_string(view->buffer, beginning_of_next_line, whitespace_len)) return false;
...
bool ce_vim_verb_set_mark(CeVim_t* vim, const CeVimAction_t* action, CeVimMotionRange_t motion_range, CeView_t* view,
                          CeVimBufferData_t* buffer_data, const CeConfigOptions_t* config_options){
